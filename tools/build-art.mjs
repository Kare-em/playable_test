...
const PLAN = [
  ...
  // Пузырь эмоции рисуется диаметром максимум 34 px, на ретине — 68.
  // 128 берём с запасом, файлы всё равно по паре килобайт. Части листа
  // (bubblepart-*) в сборку не идут: они исходник для пересборки.
  ...['cheer', 'angry', 'worry'].map((m) => ({ key: `bubble-${m}`, file: `bubble-${m}.webp`, max: 128 }))
];
